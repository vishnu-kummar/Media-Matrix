...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      current.clients = Math.min(
        current.clients + Math.ceil(targets.clients / steps),
        targets.clients
      );
      current.years = Math.min(
        current.years + Math.ceil(targets.years / steps),
        targets.years
      );
      current.media = Math.min(
        current.media + Math.ceil(targets.media / steps),
        targets.media
      );
  ...
  return (
    <div className="bg-white min-h-screen">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="animate-fadeInLeft">
              ...
              <p className="text-xl text-gray-700 mb-8 leading-relaxed">
                An Advertising company which helps Brands in optimizing their
                existing products/services and use it to buy Media solutions
                (ATL/BTL) on Barter. We specialize in providing Media solutions
                across India in the form of cash/barter. Our goal is to provide
                an innovative tool to companies where they use their existing
                excess inventory/services rather than selling at a discounted
                rates in market to optimally using it to increase the reach of
                the product through Advertisement on Barter basis.
              </p>
      ...
      {/* Mobile-only: Valued Clients → Services → Contact */}
      {isMobile && (
        <>
          {/* Our Valued Clients */}
          <div className="py-16 bg-white">
